...
export async function POST(req: NextRequest) {
  try {
    ...
    // Execute atomic attempt submission via stored procedure
    ...
    if (!rpcError && atomicResult) {
      ...
      // Fallback: application-level transaction with rollback guard
      if (rpcError) {
      ...
      try {
        // Fetch concept questions for coverage calculation
        ...
        // Fetch user attempt history for this concept
        ...
        // Rollback: delete attempt if mastery update fails
    ...
    // Trace root cause against decayed origin concept if review question failed
